...
def execute_should_i_work_here(query: str):
    """
    Skill: Should I work here - Generates a career recommendation using the Gemini API.
    """
    if not model:
        raise Exception("AI model failed to initialize due to missing or invalid API key.")

    prompt = f"""
You are my career consultant.

TASK:
1) Search Glassdoor, Indeed, and other reputable review sites for {query}.
2) Analyze employee feedback (past and present), culture, pay, benefits.
3) Compare against competitors in the same field.
4) Provide brutally honest recommendation: should the user work or continue working here?
5) If competitors are better, list current open positions with links.

Return the output in a concise, clear format.
"""
    # Using Google Search grounding tool for up-to-date company data
...
def execute():
    ...
    try:
        ...
        result = execute_should_i_work_here(query)
